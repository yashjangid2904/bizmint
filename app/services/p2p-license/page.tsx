"use client";

import React from "react";
import { motion } from "framer-motion";
import { 
  Zap, 
  Users, 
  ShieldCheck, 
  Globe, 
  ArrowRight,
  Clock,
  CheckCircle2,
  Wallet
} from "lucide-react";
import Link from "next/link";

const P2PLicensePage = () => {
  const features = [
    { title: "NBFC-P2P Status", desc: "Obtaining the specialized NBFC license for Peer-to-Peer lending.", icon: Zap, color: "text-amber-500" },
    { title: "Platform Compliance", desc: "Auditing and ensuring the lending platform follows RBI's tech norms.", icon: Globe, color: "text-blue-600" },
    { title: "Escrow Integration", desc: "Setting up the mandatory escrow mechanism for fund flow management.", icon: Wallet, color: "text-emerald-500" },
    { title: "Lender/Borrower Cap", desc: "Advisory on managing aggregate limits for lenders and borrowers.", icon: Users, color: "text-purple-600" },
  ];

  const process = [
    { name: "Tech Audit", desc: "Verification of the platform architecture and security protocols." },
    { name: "In-principle", desc: "Filing for the initial approval to setup the platform." },
    { name: "Final COR", desc: "Applying for final registration after setup and audit completion." },
    { name: "Ongoing Audit", desc: "Maintenance of statutory returns and reporting as an NBFC-P2P." },
  ];

  return (
    <main className="min-h-screen bg-white dark:bg-[#030014]">
      {/* Hero */}
      <section className="relative pt-32 pb-20">
        <div className="absolute inset-0 z-0 overflow-hidden">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_50%_-20%,#f59e0b15_0%,transparent_50%)]" />
        </div>

        <div className="relative z-10 mx-auto max-w-7xl px-6">
          <div className="flex flex-col items-center text-center">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="inline-flex items-center gap-2 rounded-full border border-amber-500/20 bg-amber-500/5 px-4 py-1.5 text-sm font-semibold text-amber-600"
            >
              Fintech Licensing
            </motion.div>
            <motion.h1 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1 }}
              className="mt-8 font-outfit text-5xl font-black text-zinc-900 dark:text-white sm:text-7xl leading-tight"
            >
              NBFC-P2P <br />
              <span className="text-amber-500">Lending License</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2 }}
              className="mt-8 max-w-2xl text-lg text-zinc-600 dark:text-zinc-400"
            >
              The specialized roadmap for crowdfunding and P2P lending platforms in India. 
              We blend tech expertise with regulatory precision.
            </motion.p>
          </div>
        </div>
      </section>

      {/* Grid */}
      <section className="py-20 bg-zinc-50 dark:bg-zinc-900/30">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
            {features.map((item, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
                className="rounded-3xl border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-900"
              >
                <div className={`mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-50 dark:bg-zinc-800 ${item.color}`}>
                  <item.icon size={24} />
                </div>
                <h3 className="text-xl font-bold text-zinc-900 dark:text-white">{item.title}</h3>
                <p className="mt-4 text-sm text-zinc-600 dark:text-zinc-400">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Timeline */}
      <section className="py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-col gap-16 lg:flex-row">
            <div className="lg:w-1/3">
              <h2 className="font-outfit text-4xl font-bold text-zinc-900 dark:text-white">Fintech Roadmap</h2>
              <p className="mt-6 text-zinc-600 dark:text-zinc-400">
                Registering a P2P platform requires satisfying both financial NOF 
                and IT security standards from RBI.
              </p>
              <div className="mt-10 flex items-center gap-4 p-6 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20">
                <ShieldCheck className="text-amber-500" size={32} />
                <div>
                  <div className="font-bold text-zinc-900 dark:text-white">Compliance Guard</div>
                  <div className="text-sm text-amber-600">IS Audit Integrated</div>
                </div>
              </div>
            </div>
            <div className="lg:w-2/3">
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                {process.map((step, i) => (
                  <div key={i} className="relative p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800">
                    <div className="absolute top-8 right-8 text-4xl font-black text-zinc-100 dark:text-zinc-800">0{i+1}</div>
                    <h4 className="text-xl font-bold text-zinc-900 dark:text-white">{step.name}</h4>
                    <p className="mt-4 text-sm text-zinc-600 dark:text-zinc-400">{step.desc}</p>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="rounded-[3rem] bg-zinc-900 p-12 text-center dark:bg-amber-600">
            <h2 className="font-outfit text-4xl font-bold text-white">Scale Your P2P Platform</h2>
            <p className="mt-6 text-zinc-400 dark:text-amber-100 max-w-xl mx-auto">
              Connect with our fintech licensing experts for a pre-eligibility review of 
              your platform architecture.
            </p>
            <Link
              href="/contact?service=p2p-license"
              className="mt-10 inline-flex items-center gap-2 rounded-full bg-white px-8 py-4 text-lg font-bold text-zinc-900 transition-all hover:bg-zinc-100 active:scale-95"
            >
              Consult an Expert
              <ArrowRight size={20} />
            </Link>
          </div>
        </div>
      </section>
    </main>
  );
};

export default P2PLicensePage;
